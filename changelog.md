...
## 2020
### 2020-3月初
- 9~10日，据多人报告，各个维度绝大多数现有生物失去AI（自主行动能力），原因不明。*凤凰卷* 认为，唯一解决方法是寻求新生成的生物。
  + 科技特区等依赖生物的生产设施停摆。
...
- ~~据多人报告，NyaaUtils附魔命令不再可用。~~
  ...
  + 「贤者の石」的翻新版本（有1.13以来的新附魔）出现，据悉可以在一些新型道具上使用。
- 31日，自定义昵称命令`/nick`开始支持非ASCII字符。~~颜文字使用可能~~
- 29日，出于性能考虑，游戏规则 `maxEntityCramming` （生物在同一位置允许推挤的实体数）[调降至 4](https://github.com/NyaaCat/wiki/commit/1e7cbac5ad5000c8ff5821156a8ec6f13584ea72)。
  + 高密度养殖场受影响较大。
