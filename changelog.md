...
- 喵窝主服务器**升级至 1.16.4 版本。**
  - PvP、NFS 等子服务器也已同步升级。
- HamsterEcoHelper 插件得到重构，多个命令用法发生改变。[详见](space/plugins/hamsterecohelper)
  - `/heh` 被 `/h` 取代。
  - 新增展示框商店，可以展示店内随机物品，并允许玩家直接购买展示框中的物品（一次购买一个）。
- NyaaUtils 插件的展示框保护功能被重写，现在依赖于原版的[展示框保护功能](https://minecraft-zh.gamepedia.com/%E7%89%A9%E5%93%81%E5%B1%95%E7%A4%BA%E6%A1%86?variant=zh#.E6.96.B9.E5.9D.97.E5.AE.9E.E4.BD.93)了。
- 由于 API 限制，**所有旧版动力燃料胶囊和经验瓶全部失效。**请等待管理组的补偿方案。
- 主服务器三个非原版维度 `EpicWorld` `EpicNether` `EpicEnd` 被重置。
- 聊天新增 RGB 彩色文本支持，支持 HTML 表记的颜色代码，如 `&#66ccff好喝的水`。

玩家发现的一些变化：

- 通过铁砧修复/附魔名称中带有彩色文本及样式的物品时，名称的彩色文本及样式不再会丢失。
- 通过 `/nu se sign` 命令设置的彩色告示牌不再工作，贴上的告示牌不会具有颜色和样式。

### 2020-09-04 服务器物理架构更新
- 喵窝主服务器迁移至一台新机器，采用了 Core i9-9900K CPU。
- 活动专用服务器 `act` 也迁移至另一新机器，采用了 Core i5-10600K CPU。
- PvP、NFS、美羽实验室迁移到了主服务器原先的平台上。该平台采用了 Xeon E3-1280 v5 CPU。
- [Terraria 游戏服务器](terraria/server) 迁移到了活动专用服务器原先的平台上。该平台采用了 Xeon E3-1271 v3 CPU。
