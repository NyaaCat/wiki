...
命令 `/heh requisition req [物品 ID] [单价（整数）] [数量（整数）]` 可发起征购。  
如果你手里已经有希望征购的物品，可以用 `/heh requisition req hand [单价（整数）] [数量（整数）]`——即「物品 ID」为 `hand`。

可用短命令 `/hreq [物品 ID] [单价（整数）] [数量（整数）]` 替代。
...
* 右键点击「销售」（SELL）牌子即可打开商店界面。单击购买一个，按住 Shift 单击则购买一整个槽。
...
收到账单后 30 秒内，可予以取消。只需使用命令 `/heh transaction cancel [账单 ID]`。  
若 30 秒内未支付或者取消账单，过后即**自动支付账单**。
