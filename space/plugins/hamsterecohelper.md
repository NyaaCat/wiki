...
!>**注意：如果购买/拍到物品时背包已满，物品将会保存至末影箱中，末影箱已满将会保存至临时储存，需要使用 `/heh storage` 查看、获取物品。**
...
然后要选择一个装有抽奖物品的箱子。准星对着想要使用的箱子，使用命令 `/heh chest lotto`，即可设置乐透箱。
...
首先创建一个物品展示框，然后准星对准展示框，使用命令 `/heh frame set` 即可设置为展示框商店。

使用 `/heh frame remove` 可以移除展示框商店。
