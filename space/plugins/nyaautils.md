...
可以在聊天栏、告示牌、本插件所有命名功能（前后缀、重命名）等处使用多彩文字。
...
可以修改在游戏内聊天框中显示的名称前后文字，支持[样式代码](#多彩文字)。建议包含可方便认识你的中文昵称，可先在聊天窗口测试完后再输入命令，以免浪费节操。
...
* 展示框物品描述
  被保护的展示框还可以设定一些描述文本，在右键点击时一并显示。只需要对准已经被保护的展示框输入 `/nu exhibition desc <行数> <文本>` 即可。注意行数从 0 开始计数，文本不支持颜色代码。文本中若包含空格，需要用反引号 <code>&#96;</code> 将文本内容括起来。
...
重命名支持[格式代码](#多彩文字)。  
如果需要在物品名称中包含空格，请使用半角反引号 <code>&#96;</code> 包围（此法同样适用于更改前后缀）。例如 
...
* 总开关：`/nu lp`
  ...
  - 关闭时插件不生效，掉落物直接掉在地上。
* 拒绝原版物品模式：`/nu lp re`
  - 该模式下仅将**带有 lore** 的物品自动放入背包；
  - **不带有 lore** 的物品即使走过也不会被拾取；然而，潜行时可以拾取。
* 忽略原版物品模式：`/nu lp ig`
  - 该模式下仅将**带有 lore** 的物品自动放入背包；
  - 走过**不带有 lore** 的物品也会拾取。
* 包括原版物品模式：`/nu lp ac`
...
- 如果需要包含空格，请使用半角反引号 <code>&#96;</code> 包围。
- 在放置前，可无限次预览内容、快速修改某一行内容。
- 无论手持多少告示牌，其均被同时编辑。
- 编辑后，内容会以 NBT 标签的形式存储，持久保存，直至被放置。  
...
可为特定的区域（通常是村落或规划区）建立领域提示，当玩家进入相应区域时，屏幕会提示你所进入区域的名称。

普通玩家可随时使用 `/nu realm info` 命令查询自己所在领域的信息。

关于如何设置领域等信息，请移步至 [这里](nyaa/economic?id=领域规划)。
