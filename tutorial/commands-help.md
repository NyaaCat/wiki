...
| `/ptt ac` | 一键领取所有在线任务奖励 | |
| `/u redbag create [fixed\|lucky] [单份金额\|总金额] [数目] [口令（可选）]` | 派发一次红包 | “fixed”可使每个红包金额相等；<br />“lucky”可使每个红包份量不一 |
| `/hm` | 访问天喵商城 | |
| `/hm offer [单价]` | 将手中物品上架到天喵商城 | 收费 |
| `/hm my` | 浏览并管理自己的商店 | |
| `/hm my offer [单价]` | 将手中物品上架到自己的牌子商店 | |
| `/hm my shops` | 列出自己所有木牌商店的位置 | |
| `/ns list` | 列出自己所有 [喵店](/tutorial/plugins/nyaashop.md) 的位置 | |
...
**主条目：[HamsterEcoHelper 插件指南](legacy/nyaa/tutorial/plugins/hamsterecohelper.md)**
...
| `/u mail box set` | 创建邮箱 | 执行后，右键点击要用作收件箱的箱子<br />收费 |
|`/u mail box clear` |删除自己的邮箱 | |
|`/u mail box info`	| 查看自己的邮箱信息 | |
|`/u mail sendto [玩家 ID]` | 发送手中的物品 | 收费 |
...
|`/u xp quicktake [经验值]`	|指定每次从附魔瓶快速提取的经验数量	| |
|`/u news` | 接收所有在离线或挂机（afk）时的未读消息 | 目前支持天喵商城、邮箱服务、出售/征购商店 |
| `/msg [玩家 ID] [正文]`<br />`/m [玩家 ID] [正文]`<br />`/t [玩家 ID] [正文]` | 与指定玩家私聊 | 只有目标玩家在线时，其才会有机会看到 |
...
| P 键 | 列出在当前服务器的玩家，并可屏蔽某人的消息 | 仅显示游戏 ID；该按键可在游戏设置中自定义 |
