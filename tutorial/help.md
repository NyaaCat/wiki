...
| `/nu realm info` | 显示当前自己所在区域信息 | 包括区域名称和坐标范围 |
| `/msg [玩家 ID] [正文]` | 与指定玩家私聊 | 只有目标玩家在线时，其才会有机会看到 |
|`/mail send [玩家 ID] [正文]`	|向指定玩家发送私信 |正文加玩家ID字数限制**244字**|
|`/mail read`	|阅读新私信	 ||
|`/mail clear`	|清空私信箱	 ||
|`/nick [昵称]` | 修改在主服务器的昵称 | 优先于游戏ID显示，但不影响之<br />昵称为“off”时，移除昵称 |
| `/rota accept` | 强制刷新服务器资源包 ||
| `/rota ignore` | 忽略服务器资源包更新提示 ||
...
| 命令| 说明 | 备注 |
| - | - | - |
| `/list`<br>`/plist` | 列出当前在主服务器的玩家 ||
| `/glist` | 列出在各个服务器的**喵窝**玩家 | 仅显示游戏 ID |
| `/tps` | 显示服务器近 10 秒经过的游戏刻 | 每秒 20 刻为正常 |
| `/ping` | 显示连接到服务器的延迟 | 统计近 10 秒数据 |
| `/yasui status`| 列出各维度已加载区块、实体数量 ||
| `/co i` | 打开日志查看器，左键点方块、右键点箱子查看变更记录。<br>再输入一次此命令关闭查看器回到正常生存状态。 | 在怀疑发生失窃、破坏等情况时使用。 |
