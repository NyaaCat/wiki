...
| `/ptt ac` | 领取刚刚提示的PTT奖励 | |
| `/h m`<br>`/hm` | 打开世界商店 | |
| `/h m [单价]`<br>`/hm [单价]` | 将手中的物品上架到世界商店 | 收费 |
| `/h req [数量] [单价]`<br>`/hreq [数量] [单价]` | 发起征购（手上的物品） | |
| `/h sell [数量（可选）]`<br>`/hsell [数量（可选）]` | 手持征购中物品，响应征购 | |
| `/h auc [起步价] [步进价] [保留价（可选）]`<br>`/hauc [起步价] [步进价] [保留价（可选）]` | 发起拍卖，拍卖手中的物品 | |
| `/h bid [价格]`<br>`/hbid [价格]` | 在拍卖中出价 | 价格**留空**或为“min”时，跟进可允许的最低价格 |
| `/h storage` | 查看/取回暂存于系统的物品 | |
| `/h shop sell [单价]` | 上架商品到你的木牌商店 | 执行前，对准自己的商店“SELL”木牌 |
| `/h chest req` | 设置收购存储箱 | |
| `/h shop buy [单价]` | 将手中的物品添加到收购列表 | 执行前，对准自己的商店“BUY”木牌 |
| `/h chest lotto` | 设置抽奖奖池存储箱 | |
| `/h frame set` | 设置展示框商店 | |
| `/h frame remove` | 移除展示框商店 | |
| `/h search [关键词] [选项:值]` | 搜索世界木牌商店中的商品 | |
| `/h search page [页码]` | 搜索结果页翻页 | |
| `/h sellto [玩家 ID] [总价]`<br>`/hsellto [玩家 ID] [总价]` | 向指定玩家出售手中物品，并发送账单 | |
| `/h pay [账单 ID]`<br>`/hpay [账单 ID]` | 支付指定账单，并收货 | 收费 |
| `/h cancel [账单 ID]` | 取消指定账单 | |
