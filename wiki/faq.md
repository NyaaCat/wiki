...
喵窝服务器不使用 Mod，各种自定义物品均通过插件实现。这种操作固然可行，但必须基于某一种原版物品为原型，且其**原版物品特性难以被覆盖**（例如某些箭外形的 RPGitem 物品可以当成普通的箭射出去，漏斗外形的物品可以被放入矿车等）。
...
这种情况下，可以尝试使用离线资源包。这是最新版本离线材质包的下载地址：
...
下载以后，按照正常加载第三方资源包的方法加载喵窝资源包，并将喵窝服务器的「使用服务器资源包」关闭即可。
...
如果只考虑解决物品栏选择的问题，可以在设置中将**鼠标灵敏度调到最大**，这样物品栏选择体验就与 Windows 一致了。
