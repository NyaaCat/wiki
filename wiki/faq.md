...
视情况而定。对于超出原版的道具，请注意：**所有 RPGItems 已经失效，请购买前尽量咨询商家，现有以下建议**
1. 所有的标明伤害，使用方法等的特殊装备已经失效，这些装备属于 RPGItems ，这些装备已经失效。
2. 由于 NPC 被移除，至 2021 年 7 月 17 日为止所有活动的代币已经失效。
3. 包含若干原版附魔的装备道具（超出原版等级限制的）可用。
4. 描述最后包含**“Unbreakable（无法破坏）”**的道具可用。
   + 含有此的盔甲不包含在内：受伤时，其只能以附魔吸收伤害，使保护效果打折扣。
5. 新版活动限定道具可用，不早于 2019 年国庆节活动。
6. 服务器[周年纪念道具](space/items/anniversary-gifts.md)。
