...
#### 服务器资源包加载缓慢/启用后不断崩溃，怎么办？ :id=server-resourcepack-dl
...
3. 如使用了 Optifine，其尝试应用资源包从未定义的天空颜色将会出错，进而使客户端闪退。
...
有些是，也有些在创造模式下建造。不过，[2019 年 2 月 1 日](changelog?id=_2019-2-1【用户组改革】) 以后，创造权限已不再开放使用。

无尽地狱世界以及活动服务器的建筑，通常是在 [专门服务器](wiki/server-network?id=nyaacat-unlimited-build-works) 用创造模式建造的。专门服务器的背包、权限等与主服务器隔离，不会影响游戏平衡。
