...
Nyaa UBW 是喵窝的创造服务器，所有玩家均为创造权限。同时还设置独立的 `test` 服务器供玩家练习和测试，**测试用世界没有备份，请不要建造重要工程**。  
除测试服务器外，UBW 通常仅对特定人员按需开放。

* **服务器代号** `ubw` `ubw1` `ubw2`...（按需开放）
* **常设测试服务器代号** `test`
* 请遵守喵窝的通用守则。
...
~~该服务器现已升级 1.14.2~~ 。除**特定测试人员**外，暂无法使用游戏内 `/server` 命令进入。  
开放测试时间未定。

*   **服务器代号** `plugin_test` ub 
或`dev1` `dev2` ...
